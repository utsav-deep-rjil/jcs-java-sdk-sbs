...
public class JCSComputeTest {
	...
	*//**
	...
	private static void init() {
		...
		JCSCredentials credentials = new JCSCredentials() {
			...
			public String getJCSAccessKey() {
				return PropertiesReader.getProperty("ACCESS_KEY");

			}

			@Override
			public String getJCSSecretKey() {
				return PropertiesReader.getProperty("SECRET_KEY");
			}
	...
	//Covers describe snapshot test
	private int getSnapshotsCount() {
		try {
			...
			Document document = builder.parse(new ByteArrayInputStream(describeSnapshotsResult.toString().getBytes("UTF8")));
			Element root = document.getDocumentElement();
			NodeList sublist = root.getElementsByTagName("snapshotSet");
			assertTrue(sublist.getLength() > 0);
			Element snapshotSet = (Element)sublist.item(0);
			int itemsCount = snapshotSet.getElementsByTagName("item").getLength();
			
			NodeList items = snapshotSet.getElementsByTagName("item");
			...
			System.out.println("Total completed snapshots = " + completedSnapshots);
	...
	//Covers describe volume test
	private int getVolumesCount() {
		try {
			...
			for(int itr = 0; itr < itemsCount; itr++){
				Element item = (Element)items.item(itr);
				if(item.getElementsByTagName("status") == null){
					continue;
				}
				String status = item.getElementsByTagName("status").item(0).getTextContent();
	...
	private String getSnapshotStatus(String snapshotId){
		try {
			...
			for(int itr = 0; itr < itemsCount; itr++){
				Element item = (Element)items.item(itr);
				String currSnapshotId = item.getElementsByTagName("snapshotId").item(0).getTextContent();
	...
	public void createAndDeleteTest() {
		try {
			...
			int volumeCountAfterCreation = getVolumesCount();
			assertThat("create volume test 1",initialVolumeCount + 1, equalTo(volumeCountAfterCreation));
			...
			CreateSnapshotRequest createSnapshotRequest = new CreateSnapshotRequest().withVolumeId(createVolumeResult.getVolume().getVolumeId());
			CreateSnapshotResult createSnapshotResult = jcs.createSnapshot(createSnapshotRequest);
			while(!getSnapshotStatus(createSnapshotResult.getSnapshot().getSnapshotId()).equals("completed"))
				;
			System.out.println(createSnapshotResult.toString());
			int snapshotCountAfterCreation = getSnapshotsCount();
			assertThat("create snapshot test",initialSnapshotCount + 1, equalTo(snapshotCountAfterCreation));
			...
			int finalVolumeCount = getVolumesCount();
			...
			createVolumeRequest = new CreateVolumeRequest().withSnapshotId(createSnapshotResult.getSnapshot().getSnapshotId());
			...
			assertThat("create volume test 2",initialVolumeCount + 1, equalTo(volumeCountAfterCreation));
			assertThat("create volume test 2 (snapshotId)",createVolumeResult.getVolume().getSnapshotId(),equalTo(createSnapshotResult.getSnapshot().getSnapshotId()));
			assertThat("create volume test 2 (size)",createVolumeResult.getVolume().getSize(),equalTo(createSnapshotResult.getSnapshot().getSize()));
			...
			while(!getVolumeStatus(createVolumeResult.getVolume().getVolumeId()).equals("deleting"))
				;
			finalVolumeCount = getVolumesCount();
			assertThat("delete volume test 2",volumeCountAfterCreation - 1, equalTo(finalVolumeCount));
			...
			while(getSnapshotStatus(createSnapshotResult.getSnapshot().getSnapshotId()).equals("deleting"))
				;
			int finalSnapshotCount = getSnapshotsCount();
